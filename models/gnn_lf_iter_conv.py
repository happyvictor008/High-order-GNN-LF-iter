...
class GNN_LF(nn.Module):
    """
    High-order graph convolution layer 
    """

    def __init__(self, in_features, out_features, adj, bias=True,):
        super(GNN_LF, self).__init__()
        ...
        self.base=self.alpha*self.mu+1-self.alpha
    ...
    def forward(self, input1, H):
        ...
        h0 = torch.matmul(((1+self.alpha*self.mu-2*self.alpha)/self.base)*A_0,input1)+((self.alpha*self.mu)/self.base)*H+torch.matmul(((self.alpha-(self.alpha*self.mu))/self.base)*A_0,H)
        #print('h_0===========================',h0.size())
        #print('w_0===========================',self.W[0].size())

        #output_0 = torch.matmul(h0,self.beta*self.W[0])
        output_0 = torch.matmul(h0,self.W[0])
        #print('+++++++++',h0.size())
        #print('+++++++++++++++',self.W[0].size())
        #print("output0----------",output_0.size())
        #output_0 = h0*(1-beta)+h0_2

        h1 = torch.matmul(((1+self.alpha*self.mu-2*self.alpha)/self.base)*A_1,input1)+((self.alpha*self.mu)/self.base)*H+torch.matmul(((self.alpha-(self.alpha*self.mu))/self.base)*A_1,H)
        output_1 = torch.matmul(h1,self.W[1])
        #h1= torch.matmul(h1,(1-beta)*)
        #h1_2 = torch.matmul(h1,beta*self.W[1])
        #output_1 = h1+h1_2

        h2 = torch.matmul(((1+self.alpha*self.mu-2*self.alpha)/self.base)*A_2,input1)+((self.alpha*self.mu)/self.base)*H+torch.matmul(((self.alpha-(self.alpha*self.mu))/self.base)*A_2,H)
        output_2 = torch.matmul(h2,self.W[2])
        #h2_2 = torch.matmul(h2,beta*self.W[2])
        #output_2 = h2*(1-beta)+h2_2

        #input
        h3 = torch.matmul(((1+self.alpha*self.mu-2*self.alpha)/self.base)*A_3,input1)+((self.alpha*self.mu)/self.base)*H+torch.matmul(((self.alpha-(self.alpha*self.mu))/self.base)*A_3,H)
        output_3 = torch.matmul(h3,self.W[3])
