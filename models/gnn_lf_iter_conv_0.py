...
class GNN_LF_0(nn.Module):
    """
    High-order graph convolution layer 
    """

    def __init__(self, in_features, out_features, adj, bias=True,):
        super(GNN_LF_0, self).__init__()
    ...
    def forward(self, H):
        ...
        A_0 = -9e15 * torch.ones_like(self.adj_0).to(H.device)
        A_1 = -9e15 * torch.ones_like(self.adj_1).to(H.device) # without self-connection 
        A_2 = -9e15 * torch.ones_like(self.adj_2).to(H.device)
        A_3 = -9e15 * torch.ones_like(self.adj_3).to(H.device)
        ...
        h0 = torch.matmul(((1-self.mu)/self.base)*A_0,H)+((self.mu)/self.base)*H
        #print('h_0===========================',h0.size())
        ...
        h1 = torch.matmul(((1-self.mu)/self.base)*A_1,H)+((self.mu)/self.base)*H
        output_1 = torch.matmul(h1,self.W[1])
        #h1= torch.matmul(h1,(1-beta)*)
        #h1_2 = torch.matmul(h1,beta*self.W[1])
        #output_1 = h1+h1_2

        h2 = torch.matmul(((1-self.mu)/self.base)*A_1,H)+((self.mu)/self.base)*H
        output_2 = torch.matmul(h2,self.W[2])
        #h2_2 = torch.matmul(h2,beta*self.W[2])
        #output_2 = h2*(1-beta)+h2_2

        #input
        h3 = torch.matmul(((1-self.mu)/self.base)*A_1,H)+((self.mu)/self.base)*H
