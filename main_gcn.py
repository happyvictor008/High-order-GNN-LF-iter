...
from models.gnn_lf_iter import GNN_LF_Iter
...
def main(args):
    ...
    adj,edges_index = adj_mx_from_skeleton(dataset.skeleton())
    #print(dataset.skeleton().shape)

    #print(adj.shape[0])
    #print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    #print(adj.shape)
    #model_pos = HGCN(adj, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout, nodes_group=dataset.skeleton().joints_group() if args.non_local else None).to(device)
    #model_pos = HGCNII(adj, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout, nodes_group=dataset.skeleton().joints_group() if args.non_local else None).to(device)
    model_pos = GNN_LF_Iter(adj, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout, nodes_group=dataset.skeleton().joints_group() if args.non_local else None).to(device)
    ...
    if args.resume or args.evaluate:
        ...
        lr_now = args.lr
        import re
        ckpt_dir_path = path.join(args.checkpoint, re.sub('[^A-Za-z0-9]+', '', datetime.datetime.now().isoformat()))
